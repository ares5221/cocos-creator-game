var UtilsFB = cc.Class({
    ...
    statics: {
        ...
        setLeaderboardLocal(leaderboardName, score) {
            let ret = false;
            let playerArrayLocal = null;
            if (leaderboardName in UtilsFB.leaderboardMapLocal) {
                playerArrayLocal = UtilsFB.leaderboardMapLocal[leaderboardName].playerArray;
            } else {
                UtilsFB.leaderboardMapLocal[leaderboardName] = {};
                UtilsFB.leaderboardMapLocal[leaderboardName].playerArray = new Array();
                playerArrayLocal = UtilsFB.leaderboardMapLocal[leaderboardName].playerArray;
            }

            let playerInfoSelf = null;
            for (let i = 0; i < playerArrayLocal.length; i++) {
                let playerInfo = playerArrayLocal[i];
                if (playerInfo.id == FBInstant.player.getID()) {
                    playerInfoSelf = playerInfo;
                }
            }

            let scoreUpdated = false;
            if (playerInfoSelf == null) {
                playerInfoSelf = {}
                playerInfoSelf.isSelf = true;
                playerInfoSelf.id = FBInstant.player.getID();
                playerInfoSelf.playerName = FBInstant.player.getName();
                playerInfoSelf.photoUrl = FBInstant.player.getPhoto();
                playerInfoSelf.score = score;
                playerInfoSelf.rank = 0;
                playerInfoSelf.photoTexture = null;
                playerArrayLocal.push(playerInfoSelf);
                scoreUpdated = true;
            } else {
                if (score > playerInfoSelf.score) {
                    playerInfoSelf.score = score;
                    scoreUpdated = true;
                }
            }
